...
- **Global scope**: `/home/steve/linguistic-tax/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/steve/linguistic-tax/.claude/notes/`)
...
2. Glob `/home/steve/linguistic-tax/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/steve/linguistic-tax/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/steve/linguistic-tax/.claude/notes/` — works in any directory
